...
dhtmlx.proto_extend = function(){
	...
	for (var i=origins.length-1; i>0; i=i-1) {
...
dhtmlx.chart.line = {
	...
	*/
	_drawItemOfLineChart:function(ctx,x0,y0,obj,label){
		...
		if(ctx.beginPath!=null) ctx.beginPath();
		if(ctx.arc!=null) ctx.arc(x0,y0,R,0,Math.PI*2,true);
		if(ctx.fill!=null) ctx.fill();
		if(ctx.stroke!=null) ctx.stroke();
		/*item label*/
		if(label)
...
dhtmlx.chart.pie = {
	...
	*/
	_addShadow:function(ctx,x,y,R){
		...
		for(var i = shadows.length-1;i>-1;i=i-1){
			ctx.beginPath();
...
dhtmlx.DataStore.prototype={
	...
	move:function(sindex,tindex){
		...
		//if (sindex<tindex) tindex=tindex-1;	//correct shift, caused by element removing
...
dhtmlx.Canvas = {
	...
	getCanvas:function(context){
        var canvas = (this._canvas||this._prepareCanvas(this._obj));
        if(canvas.getContext!=null) return canvas.getContext(context||"2d");
		else return {};
	},
	...
	clearCanvas:function(){
        ...
        if(canvas.clearRect==null){
            canvas.clearRect = function(a,b,c,d){

            }
        }
        canvas.clearRect(0,0,this._canvas.offsetWidth, this._canvas.offsetHeight);
...
dhtmlXChart.prototype={
	...
	_drawLine:function(ctx,x1,y1,x2,y2,color,width){
		...
		ctx.lineWidth = width;
		if(ctx.beginPath!=null) ctx.beginPath();
		if( ctx.moveTo) ctx.moveTo(x1,y1);
		if(ctx.lineTo) ctx.lineTo(x2,y2);
        if(ctx.stroke!=null) ctx.stroke();
	},
